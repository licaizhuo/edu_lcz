...
from edu_backend.settings.constants import LESSON_LENGTH, IMAGE_SRC
...
class Course(BaseModel):
    ...
    def lesson_list(self):
        lesson_list = CourseLesson.objects.filter(is_show=True, is_delete=False, course_id=self.id)[:LESSON_LENGTH]
    ...
    def get_active_list(self):
        return self.activeprices.filter(is_show=True, is_delete=False,
                                        active__start_time__lte=datetime.now(),
                                        active__end_time__gte=datetime.now()).order_by("-orders", "-id")

    @property
    def discount_name(self):
        """如果当前课程参与了活动，返回当前课程所对应的优惠活动的名称 """
        name = ""
        # 当前课程所对应的活动列表
        # 会出现时区问题，可以将设置文件中的USE_TZ = True该为False
        # RuntimeWarning: DateTimeField Activity.end_time received a naive datetime
        # (2020-07-16 20:55:42.305584) while time zone support is active.
        active_list = self.get_active_list()
        if active_list:
            name = active_list[0].discount.discount_type.name
        return name
        ...
        if active_list:
            ...
            if price >= condition:
                ...
                if sale == "":  # 免费
                    ...
                    set_price_list = []
                    for sale_single in sale_list:
                        set_price, discount_price = sale_single[1:].split("-")
                        # print(set_price, discount_price)
                        if price >= float(set_price):
                            set_price_list.append(float(discount_price))
                    # print(set_price_list)
                    if set_price_list:
                        price = price - max(set_price_list)
                        # print(price)
        if price < 0:
            price = 0
        return "%.2f" % price
    ...
    def real_price(self):
        ...
        return self.get_real_price(price)

    def real_expire_price(self, expire_id=0):
        """计算购物车里的真实价格"""
        flag_price = self.price
        try:
            if expire_id > 0:
                flag_price = CourseExpire.objects.get(is_show=True, is_delete=False, pk=expire_id).price
        except CourseExpire.DoesNotExist:
            pass
        price = float(flag_price)
        return self.get_real_price(price)
    ...
    def get_level(self):
        """
        在显示课程详细的序列化器中使用
        :return: 获取难度等级所对应的中文级别
        """
        return self.get_level_display()

    @property
    def brief_html(self):
        """将课程详细的介绍中的符合条件的src进行替换"""
        return self.brief.replace('src="/media', 'src="%s/media' % IMAGE_SRC)
    ...
    def active_time(self):
        ...
        if active_list:
            ...
            end_time = active_list[0].active.end_time.timestamp()
    ...
    def comment_length(self):
        ...
        try:
            ...
            return course_comment_length
        course_comment_length = len(comment_list)
        return course_comment_length
...
class CourseDiscount(BaseModel):
    """课程优惠折扣模型"""
    discount_type = models.ForeignKey("CourseDiscountType", on_delete=models.CASCADE,
