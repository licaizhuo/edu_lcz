        ...
        <div class="main">
            ...
            <div class="condition">
                <ul class="cate-list">
                    ...
                    <li v-for="(cate,index) in list_category" :key="index" @click="category=cate.id"
                ...
                <div class="ordering">
                    ...
                    <p class="condition-result">共{{list_category.length}}个课程</p>
                </div>

            </div>
            <!-- 课程列表 -->
            <div class="course-list">
                <div class="course-item" v-for="(course,index) in list_course" :key="index">
                    <div class="course-image">
                        <img :src="course.course_img" alt="图片加载失败">
                    </div>
                    <div class="course-info">
                        <h3 @click="go_detail(course.id)">{{course.name}}
                            <span>
                                <img src="/static/image/avatar1.svg" alt="">{{course.students}}人已加入学习
                            </span>
                        </h3>
                        <p class="teather-info">{{course.teacher.name}} {{course.teacher.title}} <span>共{{course.lessons}}课时/{{course.lessons===course.pub_lessons?'更新完成':`已更新${course.pub_lessons}课时`}}</span>
                        </p>
        ...
        methods: {
            ...
            get_course_list() {
                ...
                this.$axios.get(this.$settings.HOST + "course/list_filter/", {
                    ...
                    this.list_course = res.data.results;
