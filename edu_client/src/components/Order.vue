        ...
        <div class="cart-info">
            ...
            <div class="cart-item" v-for="(course,index) in course_list" :key="index">
                <el-row>
                    ...
                    <el-col :span="4" class="course-price" v-if="course.course_price!==course.course_real_price">
                        <span>¥{{course.course_real_price}}</span><br>
                        <span style="text-decoration: line-through;">原价:¥{{course.course_price}}</span>
                    </el-col>
                    <el-col :span="4" class="course-price" v-else>
                        <span>¥{{course.course_real_price}}</span><br>
                    </el-col>
                </el-row>
            </div>
            <div class="calc">
                <el-row class="pay-row">
                    ...
                    <el-col :span="8">
                        <span class="alipay" v-model="pay_type"><img src="../../static/image/alipay2.png" alt=""></span>
                    </el-col>
                    <el-col :span="8" class="count">实付款： <span>¥{{total_price}}</span></el-col>
                    <el-col :span="4" class="cart-pay">
                        <span @click="payOrder" style="display: inline-block;width: 110px">支付宝支付</span>
                    </el-col>
                </el-row>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            get_select_course(token) {
                this.$axios.get(`${this.$settings.HOST}cart/option_all/`, {
                    headers: {
                    ...
                    this.course_list_length = this.course_list.length
                    if (this.course_list_length < 1) {
                        this.$message.error("没有课程，请至少选择一个课程，才可以进行结算")
            ...
            payOrder() {
                ...
                this.$axios.post(`${this.$settings.HOST}order/option/`, {
                    ...
                    this.$message.success("订单生成成功，请注意支付的时间")
